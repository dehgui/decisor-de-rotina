...
def get_file_sha():
    """
    Retorna o SHA atual do arquivo no GitHub (necessário ao sobrescrever via API).
    """
    if GH_TOKEN is None:
        return None
...
def fetch_csv_from_github():
    """
    Faz download do conteúdo do arquivo via API (conteúdo base64) e retorna um DataFrame.
    Evita usar RAW GitHub (que sofre cache por CDN).
    """
    if GH_TOKEN is None:
        raise RuntimeError("GH_TOKEN ausente em st.secrets")
    url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/contents/{FILE_PATH}"
    headers = {"Authorization": f"token {GH_TOKEN}"}
    r = requests.get(url, headers=headers)
    if r.status_code != 200:
        raise RuntimeError(f"Erro ao buscar arquivo no GitHub: {r.status_code}")
    data = r.json()
...
def update_github_csv(new_content, commit_message="Atualizacao automatica"):
    """
    Atualiza o arquivo activities.csv no GitHub via API.
    new_content deve ser o CSV completo (string).
    """
    if GH_TOKEN is None:
        return False
    sha = get_file_sha()
...
def load_activities_df(sha):
    """
    Cachea a leitura do CSV. A função recebe o SHA para que o cache seja invalidado
    quando o SHA mudar (ou seja, quando o arquivo no GitHub for atualizado).
    """
...
def base_reward(act, state):
    """
    Calcula a utilidade base de uma ação no estado atual.
    Penaliza incompatibilidade de ambiente, falta de dinheiro, exaustão/fome mal tratada.
    """
...
def transition(state, act):
    """
    Atualiza o estado após execução de uma atividade (uma hora).
    """
    hour, energy, hunger, emotional, money, env_pref = state
    new_hour = hour + 1
...
def V(hour, energy, hunger, emotional, money, env_pref,
    ...
    """
    Função V (programação dinâmica) que retorna (valor_total, sequência_optima)
    - mand_idx_tuple: índices de atividades obrigatórias
    - mand_rem_tuple: horas restantes para cada obrigatória
    - current_mand_pos: posição no vetor de obrigatórias em execução (ou -1)
    """
    if hour >= sleep_hour:
    ...
    if current_mand_pos != -1:
        ...
        wind_ok = str(act.get("winddown", "")).strip().lower() == "true"
        ...
        return (r + fv, [(hour, act.get("name", ""), r, float(act.get("cost", 0)))] + fs)
    ...
    for i, act in enumerate(activities):
        ...
        if total > best_val:
            best_val = total
            best_seq = [(hour, act.get("name", ""), r, float(act.get("cost", 0)))] + fs
...
with tabs[0]:
    ...
    try:
        ...
        st.error(f"Erro ao carregar activities.csv: {e}")
        df_act = pd.DataFrame(columns=["name","cost","delta_energy","delta_hunger","base_utility","environment","earliest_hour","latest_hour","winddown"])
    ...
    with col1:
        ...
        sleep_hour = st.number_input("Hora de dormir (0-23) — coloque hora após meia-noite se necessário (ex: 3)", 0, 23, 23)
    ...
    mand_selected = st.multiselect("Selecione atividades obrigatórias", names)
    mand_hours = {}
    for n in mand_selected:
        mand_hours[n] = st.number_input(f"Horas contínuas mínimas para '{n}'", min_value=1, max_value=24, value=1, key=f"mand_{n}")
    ...
    if st.button("✨ Gerar planejamento ótimo"):
        ...
        total, seq = V(start_hour, energy, hunger, emotional, money, env_pref,
                       sleep_effective, -1, 0,
                       tuple(mand_idx), tuple(mand_rem), -1, 0)
        if total < -1e5 or not seq:
            st.error("Não foi possível gerar um planejamento viável com as restrições atuais.")
        else:
...
with tabs[1]:
    ...
    st.caption("Winddown: atividades calmas adequadas às últimas horas (ex.: meditar, ler).")
    ...
    if not df.empty:
        choice = st.selectbox("Escolha a atividade para remover", df["name"].tolist())
        if st.button("Remover"):
            new_df = df[df["name"] != choice]
            ok = update_github_csv(new_df.to_csv(index=False), commit_message=f"Remover atividade: {choice}")
            if ok:
                st.success("Atividade removida e CSV atualizado no GitHub.")
                st.cache_data.clear()
                st.rerun()
            else:
...
with tabs[2]:
    sha = get_file_sha()
    try:
        df_all = load_activities_df(sha)
    except Exception as e:
        st.error(f"Erro ao carregar activities.csv: {e}")
        df_all = pd.DataFrame(columns=["name","cost","delta_energy","delta_hunger","base_utility","environment","earliest_hour","latest_hour","winddown"])
